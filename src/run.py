...
import models
import numpy as np
from traintest import train, validate, validate_wa
from utilities import AverageMeter, d_prime
import torch.nn as nn
from accelerate import Accelerator
import random
from fvcore.common.config import CfgNode
from epic_sounds.epic_data import loader # TODO: Activate after setting
import yaml
...
parser.add_argument('--imagenet_pretrain', help='if use ImageNet pretrained Vim model', type=ast.literal_eval, default='False')
parser.add_argument('--imagenet_pretrain_path', help='the path to the ImageNet pretrained model', type=str, default=None)
parser.add_argument('--imagenet_pretrain_modelkey', help='the key of the ImageNet pretrained model', type=str, default='model')
...
parser.add_argument("--bs_scale_factor", type=int, default=1, help="batch size scale factor")
parser.add_argument("--fpatch_size", type=int, default=16, help="patch size")
parser.add_argument("--tpatch_size", type=int, default=16, help="patch size")
parser.add_argument("--aum_pretrain", type=ast.literal_eval, default='False', help="pretrain aum model")
parser.add_argument("--aum_pretrain_path", type=str, default=None, help="pretrain aum model path")
parser.add_argument("--aum_pretrain_fstride", type=int, default=16, help="pretrain aum model fstride")
parser.add_argument("--aum_pretrain_tstride", type=int, default=16, help="pretrain aum model tstride")
...
parser.add_argument("--ast_pretrain", type=ast.literal_eval, default='False', help="pretrain ast model")
parser.add_argument("--ast_pretrain_path", type=str, default=None, help="pretrain ast model path")
parser.add_argument("--ast_fstride", type=int, default=16, help="pretrain ast model fstride")
parser.add_argument("--ast_tstride", type=int, default=16, help="pretrain ast model tstride")
...
parser.add_argument("--optim_path", type=str, default=None, help="optimizer state path")
...
parser.add_argument("--must_square", type=ast.literal_eval, default='False', help="must square")
parser.add_argument("--fshift", type=int, default=10, help="fshift")
parser.add_argument("--imagenet_load_middle_cls_token", type=ast.literal_eval, default='True', help="load middle cls token")
parser.add_argument("--imagenet_load_double_cls_token", type=ast.literal_eval, default='False', help="load double cls token")
parser.add_argument("--use_middle_cls_token", type=ast.literal_eval, default='True', help="use middle cls token")
parser.add_argument("--use_double_cls_token", type=ast.literal_eval, default='False', help="use double cls token")
parser.add_argument("--bimamba_type", type=str, default='v2', help="bimamba type")
parser.add_argument("--transpose_token_sequence", type=ast.literal_eval, default='False', help="transpose token sequence")
parser.add_argument("--use_end_cls_token", type=ast.literal_eval, default='False', help="use end cls token")
parser.add_argument("--if_cls_token", type=ast.literal_eval, default='True', help="if cls token")
...
if args.dataset == 'epic_sounds':
    ...
    audio_conf = {'num_mel_bins': args.melbins, 'target_length': args.audio_length, 'freqm': args.freqm, 'timem': args.timem, 'mixup': args.mixup, 'dataset': args.dataset, 'mode':'train', 'mean':args.dataset_mean, 'std':args.dataset_std,
                    'noise':args.noise, 'fshift':args.fshift}
    val_audio_conf = {'num_mel_bins': args.melbins, 'target_length': args.audio_length, 'freqm': 0, 'timem': 0, 'mixup': 0, 'dataset': args.dataset, 'mode':'evaluation', 'mean':args.dataset_mean, 'std':args.dataset_std, 'noise':False, 'fshift':args.fshift}
...
if args.model == 'ast':
    ...
    accelerator.print(f'Now train an AST with the weights from {model_name} model!')

    audio_model = models.ASTModel(label_dim=args.n_class, fstride=args.fstride, tstride=args.tstride, input_fdim=args.melbins,
                                  input_tdim=args.audio_length, imagenet_pretrain=args.imagenet_pretrain,
                                  model_name=model_name,
                                  ast_pretrain=args.ast_pretrain,
                                  ast_pretrain_path=args.ast_pretrain_path,
                                    ast_fstride=args.ast_fstride,
                                    ast_tstride=args.ast_tstride,
                                    ast_label_dim=args.ast_label_dim,
                                    must_square=args.must_square,
...
if accelerator.is_main_process and args.dataset == 'speechcommands':
